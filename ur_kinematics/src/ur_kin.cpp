...
namespace ur_kinematics {
  ...
  int getConfigurationClass(double* q_in) {
    // Compute FK
    double T_raw[16];
    double* T = T_raw;
    forward(q_in, T);
    ...
    ////// Check in which class of solution is q1
    double q1s;
    double q1[2];
    {
      double A = d6*T12 - T13; // 0P05y
      double B = d6*T02 - T03; // 0P05x
      double R = A*A + B*B;
      if(fabs(A) < ZERO_THRESH) { // 0P05y very very close to 0 => pose of link 6 assumed to be on x0 axis
        double div;
        if(fabs(fabs(d4) - fabs(B)) < ZERO_THRESH) // It should be a singularity, seems to be a line of z0 axis for Link 5
          div = -SIGN(d4)*SIGN(B);
        else
          div = -d4/B;
        double arcsin = asin(div);
        ...
        // Check the class of solution for Joint 1
        if(fabs(q[0] - q1[0]) < ZERO_THRESH){
          q1s = q1[0];
        }
        else if(fabs(q[0] - q1[1]) < ZERO_THRESH){
          cc = 1;
          q1s = q1[1];
        }
        else{
      ...
      else if(fabs(B) < ZERO_THRESH) { // 0P05x very very close to 0 => pose of link 6 assumed to be on y0 axis
        double div;
        if(fabs(fabs(d4) - fabs(A)) < ZERO_THRESH)
          div = SIGN(d4)*SIGN(A);
        else
      ...
      else if(d4*d4 > R) { // No solution
        std::cout<<"d4_sq > R => no solution\n";
        return 0;
      }
      else {
        double arccos = acos(d4 / sqrt(R)) ;
    ...
    double q5[2];
    {
      double numer = (T03*sin(q1s) - T13*cos(q1s)-d4);
      double div;
      if(fabs(fabs(numer) - fabs(d6)) < ZERO_THRESH)
        div = SIGN(numer) * SIGN(d6);
      else
      ...
      // Check the class of solution for Joint 5
      if(fabs(q[4] - q5[0]) < ZERO_THRESH){
        q5s = q5[0];
      }
      else if(fabs(q[4] - q5[1]) < ZERO_THRESH){
        cc |= 1<<1;
        q5s = q5[1];
      }
      else{
    ...
    ////////////////////////////////////////////////////////////////////////////////
    {
      double c1 = cos(q1s), s1 = sin(q1s);
      double c5 = cos(q5s), s5 = sin(q5s);
      double q6;
      ////////////////////////////// wrist 3 joint (q6) //////////////////////////////
      if(fabs(s5) < ZERO_THRESH)
        q6 = 0;
      ...
      // Check the class of solution for Joint 5
      if(fabs(q[1] - q2[0]) < ZERO_THRESH  &&  fabs(q[2] - q3[0]) < ZERO_THRESH  &&  fabs(q[3] - q4[0]) < ZERO_THRESH){
      }
      else if(fabs(q[1] - q2[1]) < ZERO_THRESH  &&  fabs(q[2] - q3[1]) < ZERO_THRESH  &&  fabs(q[3] - q4[1]) < ZERO_THRESH){
        cc |= (1 << 2);
      }
      else{
                   <<"\t q[1] = "<<q[1]<<", q2[0] = "<<q2[0]<<", q2[1] = "<<q2[1]<<"\n"
                   <<"\t q[2] = "<<q[2]<<", q3[0] = "<<q3[0]<<", q3[1] = "<<q3[1]<<"\n"
                   <<"\t q[3] = "<<q[3]<<", q4[0] = "<<q4[0]<<", q4[1] = "<<q4[1]<<std::endl;
        delete q;
        return 0;
      }
    }
